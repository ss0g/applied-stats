...
\begin{abstract}
There are many factors which could have an effect on the success of FRC teams. These include general things about the
team, such as the team's age, its budget, and its size, and also things about their robot in a specific season, such as
drivetrain type and scoring capability. The goal of this research is to determine which of these factors have the
greatest effect on a team's success. I will use data from the 2022 and 2023 seasons to analyze the effect of team age,
team budget, and drivetrain type on a team's success.
\end{abstract}
...
on a team's success. I will use data from the 2022 and 2023 seasons to analyze the effect of team age, team budget, and
drivetrain type on a team's success.
...
matches will be analyzed because the alliances are random and the data only covers scouting matches. I will use win rate
as a metric of success. The variables that I will analyze are:
\begin{enumerate}
    \item Team age
    \item Team budget
    \item Drivetrain type
\end{enumerate}
...
\subsubsection{Win rate vs. drivetrain type}

$$\text{Let } \mu_h = \text{True mean win rate of teams whose robots have holonomic drivetrains}$$
$$\text{Let } \mu_{nh} = \text{True mean win rate of teams whose robots have non-holonomic drivetrains}$$
$$H_0: \mu_h = \mu_{nh} $$
$$H_a: \mu_h \neq \mu_{nh}$$
$$\alpha = 0.05$$
$$\text{Two-sample difference in means $t$-test}$$

Random: teams were selected using a simple random sample \newline
Normal:
\begin{itemize}
    \item $n_h = 34 \Rightarrow$ Central limit theorem states that the distribution of sample means is approximately normal \newline
    \item $n_{nh} = 25 \Rightarrow$ CLT fails, need normal probability plot
\end{itemize}

```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(team_data[team_data$dt_type == "nh",], aes(sample = win_rate)) + # nolint
    geom_qq() +
    geom_qq_line() +
    labs(
        x = "Theoretical",
        y = "Sample",
        title = "Normal Q-Q plot of win rate of teams with non-holonomic drivetrains"
    )

print(dt_ttest)
```

Normal probability plot is approximately linear $\Rightarrow$ win rate of teams with non-holonomic drivetrains is approximately normally distributed

Independent:
\begin{itemize}
    \item $n_h = 34 \nless 0.1N_h$, proceeding with caution
    \item $n_{nh} = 25 \nless 0.1N_{nh}$, proceeding with caution
\end{itemize}

$$t = 1.4802$$
$$p = 0.1447 \text{ on } 53.038 \text{ df}$$

Since our $p$-value (0.1447) is greater than $\alpha$ (0.05), we fail to reject our null hypothesis. Therefore, we do not
have enough evidence to support the alternative hypothesis.

\subsubsection{Win rate vs. school expenditure per pupil}

$$\text{Let } \beta = \text{True mean slope of the relationship between win rate and school expenditure per pupil}$$
$$H_0: \beta = 0$$
$$H_a: \beta \neq 0$$
$$\alpha = 0.05$$
$$\text{$t$-test for slope}$$

Linear:

```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(eps, win_rate)) +
    geom_point() +
    labs(
        x = "School expenditure per pupil ($)",
        y = "Win rate",
        title = "Win rate vs. school expenditure per pupil"
    )
```
Scatterplot of the data does not appear linear $\Rightarrow$ proceeding with caution

Normal:

```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(sample = resid(exp_lm))) +
    geom_qq() +
    geom_qq_line() +
    labs(
        x = "Theoretical",
        y = "Sample",
        title = "Normal Q-Q plot of residuals of win rate vs. school expenditure per pupil"
    )
```

Q-Q plot is approximately linear $\Rightarrow$ residuals are approximately normally distributed

Equal Variance:

```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(fitted(exp_lm), resid(exp_lm))) +
    geom_abline(slope = 0, intercept = 0) +
    geom_point() +
    labs(
        x = "Fitted values",
        y = "Residuals",
        title = "Residuals vs. fitted values of win rate vs. school expenditure per pupil"
    )
```

Residuals appear to be randomly distributed around 0 $\Rightarrow$ variance is approximately equal
...
$$t = -1.189$$
$$p = 0.241 \text{ on } 44 \text{ df}$$

Since our $p$-value (0.241) is greater than $\alpha$ (0.05), we fail to reject our null hypothesis. Therefore, we do
not have enough evidence to support the alternative hypothesis.

\subsubsection{Win rate vs. team number}

$$\text{Let } \beta = \text{True mean slope of the relationship between win rate and team number}$$
$$H_0: \beta = 0$$
$$H_a: \beta \neq 0$$
$$\alpha = 0.05$$
$$\text{$t$-test for slope}$$
...
```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(team, win_rate)) +
    geom_point() +
    labs(
        x = "Team number",
        y = "Win rate",
        title = "Win rate vs. team number"
    )
```
...
```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data[,c(1, 5)]), aes(sample = resid(team_age_lm))) + # nolint
    geom_qq() +
    geom_qq_line() +
    labs(
        x = "Theoretical",
        y = "Sample",
        title = "Normal Q-Q plot of residuals of win rate vs. team number"
    )
```
...
```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data[,c(1, 5)]), aes(fitted(team_age_lm), resid(team_age_lm))) + # nolint
    geom_abline(slope = 0, intercept = 0) +
    geom_point() +
    labs(
        x = "Fitted values",
        y = "Residuals",
        title = "Residuals vs. fitted values of win rate vs. team number"
    )
```
...
```{r echo=F}
print(summary(team_age_lm))
```

$$t = -0.145$$
$$p = 0.886 \text{ on } 56 \text{ df}$$

Since our $p$-value (0.886) is greater than $\alpha$ (0.05), we fail to reject our null hypothesis. Therefore, we do
not have enough evidence to support the alternative hypothesis.
...
$$\hat{y} = \text{Predicted win rate}$$
$$x_1 = \text{1 if team has a non-holonomic drivetrain, 0 if team has a holonomic drivetrain}$$
$$x_2 = 1 - x_1$$
$$x_3 = \text{School expenditure per pupil}$$
$$x_4 = \text{Team number}$$

```{r model, include=F}
wr_lm <- lm(win_rate ~ dt_type + eps + team, data = team_data)
wr_lm2 <- lm(win_rate ~ dt_type + eps, data = team_data)
wr_lm3 <- lm(win_rate ~ dt_type + team, data = team_data)
wr_lm4 <- lm(win_rate ~ eps + team, data = team_data)
wr_lm5 <- lm(win_rate ~ dt_type * eps * team, data = team_data)
wr_lm6 <- lm(win_rate ~ dt_type + eps + dt_type : eps + dt_type : team + dt_type : eps : team, data = team_data)
wr_lm7 <- lm(win_rate ~ dt_type + eps + dt_type : team + dt_type : eps : team, data = team_data)
# wr_lm7 chosen
```
...
\hline
\hline
$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_3 + \beta_3 x_4$ & 0.1256 & 0.06319 & 2.012 \\
\hline
$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_3$ & 0.08679 & 0.04432 & 2.043 \\
\hline
$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_4$ & 0.03889 & 0.003939 & 1.113 \\
\hline
$\hat{y} = \beta_0 + \beta_1 x_3 + \beta_2 x_4$ & 0.03499 & -0.009898 & 0.7795 \\
\hline
$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_3 + \beta_3 x_4 + \beta_4 x_1 x_3 + \beta_5 x_1 x_4 + \beta_6 x_3 x_4 + \beta_7 x_1 x_3 x_4$ & 0.2099 & 0.06437 & 1.442 \\
\hline
$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_3 + \beta_3 x_1 x_3 + \beta_4 x_2 x_4 + \beta_5 x_1 x_4 + \beta_6 x_2 x_3 x_4 + \beta_7 x_1 x_3 x_4$ & 0.2099 & 0.06437 & 1.442 \\
\hline
$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_3 + \beta_3 x_2 x_4 + \beta_4 x_1 x_4 + \beta_5 x_2 x_3 x_4 + \beta_6 x_1 x_3 x_4$ & 0.1991 & 0.0759 & 1.616 \\
\hline
\end{tabular}
...
```{r echo=F}
print(summary(wr_lm6))
```

$$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_3 + \beta_3 x_1 x_3 + \beta_4 x_2 x_4 + \beta_5 x_1 x_4 + \beta_6 x_2 x_3 x_4 + \beta_7 x_1 x_3 x_4$$
...
$$\beta_4 = 0.00003722$$
$$\beta_5 = -0.0001234$$
$$\beta_6 = 0.000000006757$$
$$\beta_7 = 0.00000003287$$

\subsubsection{$F$-test for model utility}

$$H_0: \sum_{i=1}^7 \beta_i^2 = 0$$
$$H_a: \sum_{i=1}^7 \beta_i^2 > 0$$
$$\alpha = 0.05$$
$$\text{$F$-test for model utility}$$

Normal:

```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(sample = resid(wr_lm6))) +
    geom_qq() +
    geom_qq_line() +
    labs(
        x = "Theoretical",
        y = "Sample",
        title = "Normal Q-Q plot of residuals of win rate vs. predicted win rate"
    )
```
...
```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(fitted(wr_lm6), resid(wr_lm6))) +
    geom_abline(slope = 0, intercept = 0) +
    geom_point() +
    labs(
        x = "Fitted values",
        y = "Residuals",
        title = "Residuals vs. fitted values of win rate vs. predicted win rate"
    )
```
...
$$F = 1.442$$
$$p = 0.2175 \text{ on } 7 \text{ and } 38 \text{ df}$$

Since our $p$-value (0.2175) is greater than $\alpha$ (0.05), we fail to reject our null hypothesis. Therefore, we do
not have enough evidence to support the alternative hypothesis.
...
I thought that drivetrain would be a good predictor of win rate because it is a good indicator of a team's experience
and skill. I thought that teams with holonomic drivetrains would have higher win rates than teams with non-holonomic
drivetrains because holonomic drivetrains are more complex and require more experience to build and program. However,
the $p$-value of the $t$-test for drivetrain was 0.886, which is much greater than $\alpha$. This indicates that
drivetrain is not a good predictor of win rate.

\subsection{School Expenditure per Pupil}

School expenditure per pupil could have been a good predictor of win rate because it is a good indicator of a team's
budget and resources. I thought that teams with higher school expenditure per pupil would have higher win rates because
they would have more money to spend on their robots. However, the $p$-value of the slope $t$-test for school expenditure
per student was 0.241, which is  greater than $\alpha$. This indicates that school expenditure per pupil is not a good
predictor of win rate.

\subsection{Team age}

Team number is a good proxy for team age because team numbers are assigned sequentially. I thought that team age would
be a good predictor of win rate because it is a good indicator of a team's experience and skill. I thought that older
teams would have higher win rates than younger teams because older teams would have more experience. However, the
$p$-value of the slope $t$-test for team age was 0.886, which is much greater than $\alpha$. This indicates that team
age is not a good predictor of win rate.

\subsection{Predictive Model}

The predictive model combined the three predictors listed above into a single model. I thought that this model would be
a good predictor of win rate because it combines the three predictors listed above. However, the $p$-value of the
$F$-test for model utility was 0.2175, which is much greater than $\alpha$. This indicates that the predictive model is
not effective at predicting win rate.

\section{Conclusion}

In conclusion, I was not able to find any good predictors of win rate. This is likely because win rate is a poor metric
for measuring a team's performance. Win rate is a poor metric because it is not a good indicator of a team's skill. For
example, a team could have a high win rate because they were lucky and got paired with good teams. Conversely, a team
could have a low win rate because they were unlucky and got paired with bad teams. Therefore, win rate is not a good
metric for measuring a team's performance.

Another metric that could have been used was average alliance score. Average alliance score is a better metric than win
rate because it is a better indicator of a team's skill. For example, a team with a high average alliance score is
likely a good team because they were able to score a lot of points. Conversely, a team with a low average alliance score
is likely a bad team because they were not able to score a lot of points. Therefore, average alliance score is a better
metric for measuring a team's performance.

However, average alliance score is not a perfect metric for measuring a team's performance. Average alliance score is
flawed because it could still be affected by luck. For example, a team could have a high average alliance score because
they were lucky and got paired with good teams. Conversely, a team could have a low average alliance score because they
were unlucky and got paired with bad teams. Therefore, average alliance score is not a perfect metric for measuring a
team's performance.

The best metric in my opinion would be something based off of contributions, such as average points contributed. This
is because contributions are a better indicator of a team's skill than win rate or average alliance score. For example,
a team with a high average points contributed is likely a good team because they were able to contribute a lot of
points. Conversely, a team with a low average points contributed is likely a bad team because they were not able to
contribute a lot of points. Also, a metric based on individual contributions is less likely to be swayed by luck. Good
or bad alliances would likely have no effect on such a metric. Therefore, average points contributed is a better metric
for measuring a team's performance than win rate or average alliance score.

A metric for a team's average contribution of some value $x$ could be calculated as follows:
\[ \text{contrib}(n) = \bar{x}_n - \bar{\bar{x}} + \frac{1}{3}\bar{\bar{x}} \]
where $n$ is the team number, $\bar{x}_n$ is the average value of $x$ for team $n$, and $\bar{\bar{x}}$ is the average
value of $x$ for all teams. This metric is calculated by subtracting the average value of $x$ for all teams from the
average value of $x$ for team $n$ and adding back $\frac{1}{3} \bar{\bar{x}}$. The extra $\frac{1}{3} \bar{\bar{x}}$ is
added back with the assumptions that the grand mean $x$ is contributed equally by all 3 teams in the alliance. It can be
rewritten as follows:
\[ \text{contrib}(n) = \bar{x}_n - \frac{2 \bar{\bar{x}}}{3} \]
This metric is better than average alliance score because it is immune to lucky alliances.

One issue with this study is that it only used data from the Pacific Northwest district. This is an issue because the
Pacific Northwest district is not representative of the entire FRC community. Another issue with this study is that it
only used data from the 2022 and 2023 seasons. This is an issue because the 2022 and 2023 seasons are different from
other seasons. Every year has a different game, and different things could be advantageous in different games.
