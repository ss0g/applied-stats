...
```{r echo=F}
print(summary(wr_lm7))
```

$$\hat{y} = 0.8246 + 0.05356 x_1 - 0.0001228 x_3 + 0.00001326 x_2 x_4 - 0.00008224 x_1 x_4 + 0.00000001327 x_2 x_3 x_4 + 0.00000002331 x_1 x_3 x_4$$
$$\text{where } x_1 = \text{1 if team has a non-holonomic drivetrain, 0 if team has a holonomic drivetrain}$$
$$x_2 = \text{1 if team has a holonomic drivetrain, 0 if team has a holonomic drivetrain}$$
...
```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(sample = resid(wr_lm7))) +
    geom_qq() +
    geom_qq_line() +
    labs(
        x = "Theoretical",
...
```{r echo=F, fig.dim=c(6.5, 3.5)}
ggplot(na.omit(team_data), aes(fitted(wr_lm7), resid(wr_lm7))) +
    geom_abline(slope = 0, intercept = 0) +
    geom_point() +
    labs(
        x = "Fitted values",
...
$$F = 1.616$$
$$p = 0.1687 \text{ on } 6 \text{ and } 39 \text{ df}$$

Since our $p$-value (0.1687) is greater than $\alpha$ (0.05), we fail to reject our null hypothesis. Therefore, we do
not have enough evidence to support the alternative hypothesis.
